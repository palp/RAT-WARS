...
enum CutsceneState{begin, transition_to_black, transition_to_screen, transition_frame, text_printing, idle, end}
...
func _ready():
	...
	img_array = get_scene_slides("opening")
...
func get_scene_slides(cutscene_name) -> Array:
...
func show_next_slide(next_slide):
...
func on_screen_transition_finished():
	Log.info("should be called also")
	match current_scene_state:
		CutsceneState.transition_to_screen:
...
# TODO loading and playing music
...
func _input_next_button(event: InputEvent):
	...
	if event is InputEventMouseButton and event.is_pressed():
		if current_scene_state == CutsceneState.text_printing:
			text_node.get_node("TextSpeedTimer").stop()
			print_whole_text(text_array[string_index])
...
func show_skip():
	match skip_visible:
		...
		false:
			...
			tween.tween_callback(indicator_next.get_node("ButtonSkipHideTimer").start)
			pass
